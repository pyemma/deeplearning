...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dfc2, dW2, db2 = affine_backward(dx, fc2_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(1, self.num_layers - 1):
      n = i + 1
      self.params['W%d' % n] = np.random.normal(0.0, weight_scale, (hidden_dims[i-1], hidden_dims[i]))
      self.params['b%d' % n] = np.zeros(hidden_dims[i])
    # initial last layer
  ...
  def loss(self, X, y=None):
    ...
    dx, dW, db = affine_backward(dx, cache['fc%d_cache' % self.num_layers])
