...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    gradient[np.arange(N), y] -= 1.0
    gradient /= N

    db2 = np.sum(gradient, axis=0)
    dW2 = ReLU_out.T.dot(gradient) + reg * W2
    dReLU_out = gradient.dot(W2.T)

    dFC_out = dReLU_out * (FC_out >= 0.0)

    db1 = np.sum(dFC_out, axis=0)
    dW1 = X.T.dot(dFC_out) + reg * W1
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      batch_index = np.random.choice(num_train, min(num_train, batch_size), replace=False)
